...
-- Programs
hl.bind(mainMod .. " + R", hl.dsp.exec_cmd("pkill rofi || " .. menu)) -- toggles the menu
...
-- Slurp / Satty
hl.bind("SUPER + SHIFT + S", hl.dsp.exec_cmd("sh -c 'hyprshot -m region -o ~/Bilder/Screenshots --raw | satty --filename -'"))

-- Color pickler
hl.bind(mainMod .. " + H", hl.dsp.exec_cmd("hyprpicker -a"), { locked = true })
...
--- Use picture-in-picture mode for Firefox's PiP windows
hl.window_rule({
    name  = "firefox-pip",
    match = { 
        class = "firefox", 
...
--- Satty is used for screenshots, so we want it to float and be focused immediately when launched, but not appear in the taskbar or alt-tab list
hl.window_rule({
    name = "satty-floating",
